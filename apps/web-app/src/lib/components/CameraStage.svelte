...
<script lang="ts">
  ...
  const showPlaceholder = $derived((cameraState !== 'ready' || !cameraEnabled) && !roomName);
...
<div class="camera-stage" class:in-room={inRoom} style={`--cols: ${cols}; --rows: ${rows};`}>
  <video bind:this={videoEl} class="source-video" autoplay playsinline muted aria-hidden="true"
  ></video>
...
{#if inRoom}
  <RoomParticipantsGrid {roomName} {connectionState} {participants} />
{/if}
