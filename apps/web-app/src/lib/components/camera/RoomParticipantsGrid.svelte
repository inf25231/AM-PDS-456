...
the live <video> + effect canvas overlays. This is the exact same visual
  ...
  <script lang="ts">
    ...
    function signalIconFor(quality: string): string {
      switch (quality) {
        case "excellent": return signal4;
        case "good":      return signal3;
        case "poor":      return signal2;
        case "lost":      return signal1;
        default:          return signalUnknown;
      }
    }
    ...
    function bindMediaStream(node: HTMLMediaElement, stream: MediaStream | null) {
      ...
      apply(stream);
      return {
        update(next: MediaStream | null) { apply(next); },
        destroy() {
          node.pause();
          node.srcObject = null;
        },
      ...
      {#each participants as p (p.id)}
        <article class="tile" class:speaking={p.isSpeaking}>
          ...
          <span class="tile-name" class:speaking-name={p.isSpeaking}>
            ...
            {/if}
              <img src={signalIconFor(p.connectionQuality)} alt={`Signal: ${p.connectionQuality}`} class="signal-icon" />
          </span>
          </footer>
        </article>
      {/each}
