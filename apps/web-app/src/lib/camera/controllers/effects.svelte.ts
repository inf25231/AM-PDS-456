...
export function shouldTrackFace(state: CameraEffectsState): boolean {
    return (
        state.model.enabled ||
        state.cutouts.enabled ||
        state.showLandmarksDebug
    );
}
...
export class EffectsController {
    ...
    attachElements(elements: AttachedElements): void {
        ...
        this.composition = new CompositionController({
            ...
            renderBackground: (ctx, state) => this.drawBackgroundFrame(ctx, ctx.canvas.width, ctx.canvas.height, state),
    ...
    syncTracking(): void {
        ...
        const targetFps = this.isMobileViewport
            ? FACE_TRACKING_FPS.MOBILE
            : FACE_TRACKING_FPS.DESKTOP;
