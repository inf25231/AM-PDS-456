...
export class RoomController {
    ...
    private async connect(roomName: string, username: string): Promise<void> {
        ...
        try {
            ...
            if (joinResponse.username !== username) {
                this.opts.onInfo?.(
                    `Your name was adjusted to ${joinResponse.username}.`,
                );
            }
    ...
    private registerRoomEvents(room: Room, sessionId: number): void {
        const isActiveSession = () =>
            sessionId === this.sessionId && room === this.room;
    ...
    private async askRoomName(): Promise<string> {
        const previous =
            this.getStorage()?.getItem(ROOM_NAME_STORAGE_KEY) || '';
        ...
        return (
            window.prompt('Room name', previous || 'amphi-room')?.trim() || ''
        );
    }

    private async askUserName(): Promise<string> {
        const previous =
            this.getStorage()?.getItem(USER_NAME_STORAGE_KEY) || '';
    ...
    private qualityLabel(quality: ConnectionQuality | undefined): string {
        ...
        const label =
            typeof quality === 'number'
                ? String(
                    (ConnectionQuality as unknown as Record<number, string>)[
                        quality
                        ] || 'unknown',
                )
                : String(quality);
    ...
    private toMediaStreamTrack(track: unknown): MediaStreamTrack | null {
        ...
        const maybe = (track as { mediaStreamTrack?: MediaStreamTrack })
            .mediaStreamTrack;
