...
import { FACE_OVAL_INDICES, LEFT_EYE_INDICES, LIPS_INDICES, RIGHT_EYE_INDICES } from 'camera-core';
...
function drawCutoutRegion(
  ...
  ctx.drawImage(videoEl, sx, sy, sw, sh, -shapedW / 2, -shapedH / 2, shapedW, shapedH);
...
export function drawCameraEffectsOverlay(
  ...
  const leftEyeVideo = toVideoRegion(
    videoFaceLandmarks,
    videoEl.videoWidth,
    videoEl.videoHeight,
    LEFT_EYE_INDICES
  );
  const rightEyeVideo = toVideoRegion(
    videoFaceLandmarks,
    videoEl.videoWidth,
    videoEl.videoHeight,
    RIGHT_EYE_INDICES
  );
  const lipsVideo = toVideoRegion(
    videoFaceLandmarks,
    videoEl.videoWidth,
    videoEl.videoHeight,
    LIPS_INDICES
