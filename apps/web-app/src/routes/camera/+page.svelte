...
<script lang="ts">
    ...
    const media = new MediaController({
        ...
        onError: (message) => { banner.showError(message); },
        onMediaChanged: (reason) => {
            publish?.onMediaChanged(reason);

            if (
                (reason === 'camera-started' || reason === 'video-device-changed') &&
                room?.isConnected
            ) {
    ...
    const room = new RoomController({
        media,
        onInfo: (message) => { banner.showInfo(message); },
        onError: (message) => { banner.showError(message); },
...
<div
        class:camera-view={true}
        class:in-room={room.connectionState === "connected"}
>
    <CameraStage
            bind:videoEl
    ...
    <MediaControls>
        {#if room.connectionState === 'connected'}
            <PillButton
                    tone="danger"
                    onclick={() => room.leave({ restartMedia: true })}
            >
                Leave
            </PillButton>
        {/if}
        ...
        <div class="effects-anchor">
            <EffectsMenu
                    ...
                    onClearModel={() => { effects.setModelEnabled(false); void effects.handleUploadModel(null); }}
            ...
            >
                <ToggleIcon
                        active={effects.showPanel}
                        activeSrc={closeIcon}
                        inactiveSrc={starsIcon}
                />
            </PillButton>
        </div>
        ...
        >
            <ToggleIcon
                    active={media.microphoneEnabled}
                    activeSrc={micOn}
                    inactiveSrc={micOff}
            />
        </PillButton>
